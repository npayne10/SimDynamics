...
int ensure_socket() {
    ...
    if (sock < 0) {
        std::cerr << "rFactor2 interface: unable to open UDP socket" << std::endl;
        return -1;
    }
    sockaddr_in addr{};
    ...
    if (bind(sock, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) < 0) {
        std::cerr << "rFactor2 interface: bind failed" << std::endl;
        ::close(sock);
        sock = -1;
        return -1;
    }
...
bool Rfactor2Interface::poll(TelemetryFrame& frame) {
